...
za_critical_values = {}
c = ((0.001, -6.78442), (0.100, -5.83192), (0.200, -5.68139),
...
za_critical_values['c'] = np.asarray(c)

t = ((0.001, -83.9094), (0.100, -13.8837), (0.200, -9.13205),
...
za_critical_values['t'] = np.asarray(t)

ct = ((0.001, -38.17800), (0.100, -6.43107), (0.200, -6.07279),
...
za_critical_values['ct'] = np.asarray(ct)

def za_crit(stat, model='c'):
    """
    ...
    model : {'c','t','ct'}
        The model used when computing the ZA statistic
    ...
    ZA test statistic distribution using 100,000 replications and 2000
    data points
    """
    table = za_critical_values[model]
    y = table[:, 0]
    x = table[:, 1]
